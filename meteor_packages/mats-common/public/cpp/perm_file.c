...
main (int argc, char *argv[]) {
  ...
  CT  *newCT();
  CTPair *newCTPair();
  void get_stat1(char *stat_type,int vtime,CTPair *ctp[],int n_ctp,int min_valid_time,int max_valid_time
		 ,int pairs,float sd_limit);
  ...
  for (i = 0;;) {
	...
	sscanf(line, "%d %d %d %d %d %d %d %d %d %d",
	       &valid_time,
	       &avtime,
	       &ct0->hits,&ct0->fas,&ct0->misss,&ct0->crs,
	       &ct1->hits,&ct1->fas,&ct1->misss,&ct1->crs);
      /* printf(";%d input count is %d %d %d %d\n",i,input_count,valid_time,avtime,ct1->crs);*/
    ...
    if(avtime != last_avtime && last_avtime != -1) {
      ...
      get_stat1(stat_type,last_avtime,&ctp[i_start],n_ctp,min_valid_time,max_valid_time,pairs,sd_limit);
      i_start = i-1;
      n_ctp=1;
    }
    last_avtime = avtime;
