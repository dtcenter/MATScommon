...
/* not used at the moment, because this didn't catch an appreciable number of problem events */
int clean_data(char *stat_type,CTPair *ctp[],int n_ctp,int pairs,float sd_limit) {
  /* returns number of ctpairs for which the difference in the number of total events
     is within sd_limit*sd of the mean difference in number of total events */
  ...
  for(i=0;i<n_ctp;i++) {
    n0 = ctp[i]->ct[0]->hits+ctp[i]->ct[0]->misss+ctp[i]->ct[0]->fas+ctp[i]->ct[0]->crs;
    n1 = ctp[i]->ct[1]->hits+ctp[i]->ct[1]->misss+ctp[i]->ct[1]->fas+ctp[i]->ct[1]->crs;
    n_diff = n0-n1;
    sum += n_diff;
    sum2 += n_diff*n_diff;
  }
...
/* clean_data1 cleans hourly outliers for the desired stat (CSI, Bias, etc.). This may not be a
   very good test for stats that bounce around a lot on an hourly basis. If you want to use this in
   the future, rename this as 'clean_data', and rename the other (newer) clean_data as something else. */
int clean_data1(char *stat_type,CTPair *ctp[],int n_ctp,int pairs,float sd_limit) {
  /* returns number of ctpairs with that are "clean" (in terms of being within sd_limit*std from the mean) */
  Stat *this_s;
  ...
  printf(";n_pairs: %d, n out of range: %d (non-NAN stats: %d, %d)\n",
	 n_ctp,n_oor,n_non_bad[0],n_non_bad[1]);
  if(n_oor > 0) {
    /* put all the negative valid times at the end */
    qsort(ctp,n_ctp,sizeof(CTPair *),compareValidTimes);
    good_ctp = n_ctp - n_oor;
...
void get_stat1(char *stat_type,int avtime,CTPair *ctp[],int n_ctp,int min_valid_time,
	       int max_valid_time,int pairs,float sd_limit) {
  ...
  if(good_ctp < 10) {
    if(true_stat->val[0] > BAD_VAL/2.0 ||
      ...
      printf("%s %d %d %f %f %f %f %d %d\n",
	     stat_type,good_ctp,avtime,true_stat->val[0],true_stat->val[1],
	     true_stat->val_diff,BAD_SHORT,min_valid_time,max_valid_time);
    }
    return;
  }
  if(pairs != 0) {
    ...
    for(i=0;i<MAX_TRYS;i++) {
      stat[i] = gen_stat(gen_overall_ctp(ctp,good_ctp,1,pairs),stat_type,pairs);
    }
    ...
    for(i=0;i<0;i++) {
      printf(";%d %s: %f %f %f\n",
	     i,stat_type,stat[i]->val[0],stat[i]->val[1],stat[i]->val_diff);
    }
  ...
  if(true_stat->val[0] > BAD_VAL/2.0 ||
    ...
    printf("%s %d %d %f %f %f %f %d %d\n",
	   stat_type,good_ctp,avtime,true_stat->val[0],true_stat->val[1],
	   true_stat->val_diff,(top_95-bot_95)/(1.96*2),min_valid_time,max_valid_time);
  }
}
...
Stat *gen_stat(CTPair *ctp,char *stat_type,int pairs) {
  ...
  if(strncmp(stat_type,"CSI",3) == 0) {
    for(i=0;i<i_max;i++) {
      val_temp = (double)(ctp->ct[i]->hits + ctp->ct[i]->misss + ctp->ct[i]->fas);
      if(val_temp == 0) {
	val = BAD_VAL;
      } else {
    ...
    for(i=0;i<i_max;i++) {
      val_temp = (double)(ctp->ct[i]->hits + ctp->ct[i]->misss + ctp->ct[i]->fas + ctp->ct[i]->crs);
      if(val_temp == 0) {
	val = BAD_VAL;
      } else {
    ...
    for(i=0;i<i_max;i++) {
      val = ctp->ct[i]->hits + ctp->ct[i]->misss + ctp->ct[i]->fas + ctp->ct[i]->crs;
      /*printf("%d val is %f\n",i,val);*/
      s->val[i] = val;
    }
  }  else {
...
void printCTP(CTPair *ctp) {
  printf("%d %d %d %d %d %d %d %d\n",
	 ctp->ct[0]->hits,ctp->ct[0]->misss,ctp->ct[0]->fas,ctp->ct[0]->crs,
	 ctp->ct[1]->hits,ctp->ct[1]->misss,ctp->ct[1]->fas,ctp->ct[1]->crs);
}
