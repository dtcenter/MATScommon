...
const getAppSourceByApplication = function (application) {
  if (
    matsCollections["application"].findOne({ name: "application" }).sourceMap !==
    undefined
  ) {
...
const getAllStats = function (blockName) {
  ...
  const myRegions = Object.keys(
    myScorecard["scorecard"]["results"]["blocks"][blockName]["data"]
  );
  let myStats = new Set();
  myRegions.forEach(function (r) {
    const rStats = Object.keys(
      myScorecard["scorecard"]["results"]["blocks"][blockName]["data"][r]
    );
...
Template.ScorecardDisplay.helpers({
  application: function (blockName) {
    ...
    return myScorecard["scorecard"].plotParams.curves.find(
      (r) => r["label"] === blockName
    )["application"];
  },
  blockTitle: function (blockName) {
    ...
    const blockTitle =
      myScorecard["scorecard"]["results"]["blocks"][blockName]["blockTitle"];
    return (
      "Scorecard " +
      blockName +
  ...
  fcstlens: function (blockName) {
    ...
    let myFcstlenStrs =
      myScorecard["scorecard"]["results"]["blocks"][blockName]["fcstlens"];
    let myFcstLengths = [];
  ...
  significanceIconHTML: function (
    ...
    let fcstlenStr = Number(fcstlen) + "";
    const sigVal =
      myScorecard["scorecard"]["results"]["blocks"][blockName]["data"][region][stat][
        variable
    ...
    const scorecardSettings = {
      ...
      commonCurveParams: {
        region: e.currentTarget.dataset.region
          ? e.currentTarget.dataset.region.replace(/__DOT__/g, ".")
          : "undefined",
        "forecast-length":
          e.currentTarget.dataset.fcstlen && !blockData["forecast-type"]
            ? parseInt(e.currentTarget.dataset.fcstlen)
                .toString()
                .replace(/__DOT__/g, ".")
            : "undefined",
        statistic: e.currentTarget.dataset.stat
          ? e.currentTarget.dataset.stat.replace(/__DOT__/g, ".")
          : "undefined",
        variable: e.currentTarget.dataset.variable
          ? e.currentTarget.dataset.variable.replace(/__DOT__/g, ".")
          : "undefined",
        threshold:
          e.currentTarget.dataset.threshold &&
          e.currentTarget.dataset.threshold !== "threshold_NA"
            ? e.currentTarget.dataset.threshold.replace(/__DOT__/g, ".")
            : "undefined",
        level:
          e.currentTarget.dataset.level && e.currentTarget.dataset.level !== "level_NA"
            ? e.currentTarget.dataset.level.replace(/__DOT__/g, ".")
            : "undefined",
