...
Template.select.onRendered(function () {
  ...
  try {
    ...
    e.message =
      "Error in select.js rendered function checking to hide or disable other elements: " +
      e.message;
    setError(e);
  }
});
...
Template.select.events({
  ...
  "click .doneSelecting": function (event) {
    ...
    $("#" + this.name + "-" + this.type)
      .select2("close")
      .trigger("change"); // apply the selection choices to the select2
        ...
        if (!priorSelected.includes(selection)) {
          ...
          if (unusedOption === "") {
            ...
            Session.set(
              ...
              "The prior option: " +
                unusedOption +
                " was not selected for this selector, you must use that first."
            );
